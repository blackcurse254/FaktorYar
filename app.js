...
(function () {
  ...
  function renderNew() {
    ...
    el.innerHTML =
      ...
      '<div class="card card-pad" style="margin-bottom:16px">' +
        '<div class="eyebrow" style="margin-bottom:10px">صنف</div>' +
        '<div class="trade-grid">' + FD.TRADES.map(function (t) {
          return '<button class="trade-chip' + (inv.trade === t.id ? ' active' : '') + '" style="--tc:' + t.hue + '" data-trade="' + t.id + '">' +
            '<svg class="ico" viewBox="0 0 24 24">' + t.icon + '</svg><span>' + t.name + '</span></button>';
        }).join('') + '</div>' +
      '</div>' +
    ...
    el.querySelectorAll('[data-trade]').forEach(function (b) {
      b.addEventListener('click', function () {
        inv.trade = b.getAttribute('data-trade');
        el.querySelectorAll('[data-trade]').forEach(function (x) { x.classList.remove('active'); });
  ...
  function renderInvTable() {
    ...
    wrap.innerHTML = '<table class="tbl"><thead><tr><th>شماره</th><th>مشتری</th><th>صنف</th><th>تاریخ</th><th>مبلغ</th><th>وضعیت</th><th></th></tr></thead><tbody>' +
      list.map(function (inv) {
        var t = invTotals(inv);
        return '<tr>' +
          '<td class="mono num">' + FK.esc(inv.num) + '</td>' +
          '<td>' + FK.esc(clientName(inv)) + '</td>' +
          '<td class="flex gap-6 center">' + tradeIcoSpan(inv.trade) + '<span class="dim" style="font-size:11.5px">' + FK.esc(FD.tradeById(inv.trade).name) + '</span></td>' +
          '<td class="num">' + FK.jFormat(inv.date, 'numeric') + '</td>' +
          '<td class="num tnum">' + FK.Money.fmt(t.grand) + '</td>' +
          '<td>' + statusBadge(inv.status) + '</td>' +
          '<td><div class="row-actions">' +
    ...
    document.getElementById('printArea').innerHTML =
      '<div class="paper" id="paperNode"><div class="paper-pad">' +
        ...
        '<div class="flex between" style="gap:20px;flex-wrap:wrap">' +
          ...
          '<div><div style="font-size:11px;color:#888;margin-bottom:3px">صنف</div><div style="font-weight:700">' + FK.esc(FD.tradeById(inv.trade).name) + '</div></div>' +
        '</div>' +
  ...
  function renderCatalog() {
    ...
    el.innerHTML =
      '<div class="topbar"><div><div class="page-title display">قیمت‌نامه</div><div class="page-sub">لیست خدمات و قیمت‌ها بر اساس صنف</div></div>' +
      '<button class="btn btn-primary" id="catAddBtn">' + svg('plus') + 'افزودن قلم</button></div>' +
      '<div class="trade-grid" style="margin-bottom:16px">' + FD.TRADES.map(function (t) {
        return '<button class="trade-chip' + (UI.catalogTrade === t.id ? ' active' : '') + '" style="--tc:' + t.hue + '" data-trade="' + t.id + '">' +
          '<svg class="ico" viewBox="0 0 24 24">' + t.icon + '</svg><span>' + t.name + '</span></button>';
      }).join('') + '</div>' +
      '<div class="card"><div class="table-wrap" id="catTableWrap"></div></div>';

    el.querySelectorAll('[data-trade]').forEach(function (b) {
      b.addEventListener('click', function () { UI.catalogTrade = b.getAttribute('data-trade'); renderCatalog(); });
    });
